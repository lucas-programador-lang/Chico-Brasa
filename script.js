    ...
    return `
        <div class="review-card" id="review-${id}">
            ...
            <div class="review-body">
                ...
                <div class="reply-composer" id="reply-composer-${id}" hidden>
                    <textarea class="rating-textarea reply-textarea" id="reply-input-${id}" maxlength="280" rows="2" placeholder="Escreva uma resposta..."></textarea>
                    <div class="reply-composer-actions">
                        <button type="button" class="review-edit-cancel" data-reply-cancel="${id}">Cancelar</button>
                        <button type="button" class="review-edit-save" data-reply-send="${id}">Responder</button>
                    </div>
                </div>
            </div>
        </div>
...
function renderReplyCard(reviewId, replyId, reply) {
    const isOwner = currentReviewUser && reply.uid === currentReviewUser.uid;
    const name = escapeHtml(reply.name || 'Cliente');
    const text = escapeHtml(reply.text || '');
    const dateStr = formatReviewDate(reply.updatedAt || reply.createdAt);
    const wasEdited = reply.updatedAt && reply.createdAt && reply.updatedAt !== reply.createdAt;

    const avatar = reply.photoURL
        ? `<img class="reply-avatar" src="${escapeHtml(reply.photoURL)}" alt="">`
        : `<span class="reply-avatar-fallback">${name.charAt(0).toUpperCase()}</span>`;

    const ownerActions = isOwner ? `
        <button type="button" class="review-action-btn" data-reply-edit="${reviewId}:${replyId}">
            <i class="fa-solid fa-pen" aria-hidden="true"></i> Editar
        </button>
        <button type="button" class="review-action-btn is-danger" data-reply-delete="${reviewId}:${replyId}">
            <i class="fa-solid fa-trash" aria-hidden="true"></i> Excluir
        </button>
    ` : '';

    return `
        <div class="reply-card" id="reply-${replyId}">
            ${avatar}
            <div class="reply-body">
                ...
                <p class="review-text${wasEdited ? ' is-edited' : ''}">${text}</p>
                <div class="review-actions">${ownerActions}</div>
            </div>
        </div>
    `;
}
...
async function submitReply(reviewId, text) {
    ...
    if (!trimmed || !window.__ratingsDB) return;

    try {
        await ensureSignedIn();
    } catch (err) {
        console.error('Login necessário pra responder:', err);
        showFeedback('Entra com Google pra responder 🙂');
        return;
    }

    const { ref, push, update, serverTimestamp, db } = window.__ratingsDB;
    const newRef = push(ref(db, `reviews/${reviewId}/replies`));
    ...
    try {
        ...
        showFeedback('Resposta publicada!');
    } catch (err) {
        console.error('Não foi possível publicar a resposta:', err);
        showFeedback('Não foi possível publicar a resposta.');
    }
}

function startEditReply(reviewId, replyId) {
    const review = lastReviewsSnapshot[reviewId];
    const reply = review && review.replies && review.replies[replyId];
    const card = document.getElementById(`reply-${replyId}`);
    if (!reply || !card) return;

    const body = card.querySelector('.reply-body');
    if (body.querySelector('.review-edit-box')) return; // já editando

    const box = document.createElement('div');
    box.className = 'review-edit-box';
    box.innerHTML = `
        <textarea class="rating-textarea" maxlength="${COMMENT_MAX_LENGTH}">${escapeHtml(reply.text || '')}</textarea>
    ...
    box.querySelector('.review-edit-save').addEventListener('click', () => {
        ...
        update(ref(db, `reviews/${reviewId}/replies/${replyId}`), {
            text: newText.slice(0, COMMENT_MAX_LENGTH),
            updatedAt: serverTimestamp()
        }).then(() => {
            showFeedback('Resposta atualizada!');
        }).catch(err => {
            console.error('Não foi possível editar a resposta:', err);
...
async function deleteReply(reviewId, replyId) {
    const confirmed = await showConfirmDialog({
        ...
        message: 'Tem certeza que quer excluir sua resposta? Essa ação não pode ser desfeita.',
        okLabel: 'Excluir'
    });
    if (!confirmed) return;

    const { ref, remove, db } = window.__ratingsDB;
    remove(ref(db, `reviews/${reviewId}/replies/${replyId}`)).catch(err => {
        console.error('Não foi possível excluir a resposta:', err);
        showFeedback('Não foi possível excluir a resposta.');
    });
}
